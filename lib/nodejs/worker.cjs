...
try { ({createInvalidArgumentTypeError, createInvalidArgumentValueError} = require("../errors.cjs")); } catch { ({createInvalidArgumentTypeError, createInvalidArgumentValueError} = require("../errors.js")); }
...
// Piscina threads share PID but need unique IDs for Mocha's per-worker fixtures (e.g. Mongo DB per worker)
process.env.MOCHA_WORKER_ID = String(threadId);
