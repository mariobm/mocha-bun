...
let isMochaError;
try { ({isMochaError} = require("../errors.cjs")); } catch { ({isMochaError} = require("../errors.js")); }
...
const isBunEsm = !!process.versions.bun || typeof Bun !== "undefined";
debug(
  "assigning requireOrImport, require_module === %O, isBun=%s",
  process.features.require_module,
  isBunEsm,
);
if (isBunEsm) {
  // Bun handles .ts/.js ESM natively via import(), prefer tryImportAndRequire
  exports.requireOrImport = tryImportAndRequire;
} else if (process.features.require_module) {
