...
const { createMap, constants } = require("../utils.cjs");
const { MOCHA_ID_PROP_NAME } = constants;
let createFatalError;
try { ({createFatalError} = require("../errors.cjs")); } catch { ({createFatalError} = require("../errors.js")); }

const DEFAULT_WORKER_REPORTER =
  require.resolve("./reporters/parallel-buffered.cjs");
  ...
  run(callback, { files, options = {} } = {}) {
    ...
    (async () => {
      ...
      try {
        ...
        this.started = true;
        this._state = RUNNING;
        this.state = Runner.constants.STATE_RUNNING;
        ...
        this.state = Runner.constants.STATE_STOPPED;
