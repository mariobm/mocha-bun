...
class BunForkPool {
  constructor(maxWorkers, opts = {}) {
    ...
    this.maxTasksPerWorker =
      opts.maxTasksPerWorker ?? WORKER_POOL_DEFAULT_OPTS.maxTasksPerWorker;
    ...
    debug(
      "BunForkPool created maxWorkers=%d maxTasksPerWorker=%s maxRSS=%s",
      maxWorkers,
      this.maxTasksPerWorker,
      this.maxRSSBytes,
    );
  ...
  _createWorker() {
    ...
    const onMessage = (msg) => {
      ...
      const shouldRecycle =
        (this.maxTasksPerWorker > 0 &&
          worker.tasksRun >= this.maxTasksPerWorker) ||
    ...
    child.on("exit", (code, signal) => {
      debug(
        "Bun worker %d exit code=%s signal=%s state=%s",
        id,
        code,
        signal,
        worker.state,
  ...
  _processQueue() {
    while (this.queue.length > 0) {
      ...
      let worker = this.workers.find(
        (w) => w.state === WorkerState.IDLE && w.ready,
      );
  ...
  async terminate(force = false) {
    debug(
      "BunForkPool terminate force=%s workers=%d queued=%d",
      force,
      this.workers.length,
      this.queue.length,
    );
  ...
  stats() {
    return {
      ...
      busyWorkers: this.workers.filter((w) => w.state === WorkerState.BUSY)
        .length,
      idleWorkers: this.workers.filter((w) => w.state === WorkerState.IDLE)
        .length,
...
class BufferedWorkerPool {
  ...
  constructor(opts = {}) {
    ...
    const poolType =
      opts.pool || opts.workerType || WORKER_POOL_DEFAULT_OPTS.pool;
    const maxTasksPerWorker =
      opts.maxTasksPerWorker ?? WORKER_POOL_DEFAULT_OPTS.maxTasksPerWorker;
    const maxRSSBytes =
      opts.maxRSSBytes ?? WORKER_POOL_DEFAULT_OPTS.maxRSSBytes;
    ...
    if (poolType === "process") {
      debug(
        "using BunForkPool (process) isBun=%s maxTasksPerWorker=%s",
        isBun,
        maxTasksPerWorker,
      );
      this._pool = new BunForkPool(maxWorkers, {
        maxTasksPerWorker,
        maxRSSBytes,
      });
    } else {
  ...
  async terminate(force = false) {
    /* istanbul ignore next */
    debug(
      "terminate(): terminating with force = %s pool=%s",
      force,
      this.options.pool,
    );
  ...
  stats() {
    ...
    const busyWorkers =
      pendingTasks > 0 ? totalWorkers : Math.min(totalWorkers, 1);
    return {
      totalWorkers,
      busyWorkers,
