...
try { ({NO_FILES_MATCH_PATTERN} = require("../error-constants.cjs").constants); } catch { ({NO_FILES_MATCH_PATTERN} = require("../error-constants.js").constants); }
let lookupFiles;
try {
...
try { ({createNoFilesMatchPatternError, createMissingArgumentError} = require("../errors.cjs")); } catch { ({createNoFilesMatchPatternError, createMissingArgumentError} = require("../errors.js")); }
  const debugFallback = require("debug")("mocha:cli:lookup-files");
  function hasMagicFallback(p) { return /[*?[\]{}()!+@]/.test(p); }
  const isHiddenOnUnix = (pathname) => pathFallback.basename(pathname).startsWith(".");
  const hasMatchingExtname = (pathname, exts = []) =>
    exts.map((ext) => (ext.startsWith(".") ? ext : `.${ext}`)).some((ext) => pathname.endsWith(ext));
  lookupFiles = (filepath, extensions = [], recursive = false) => {
    ...
    if (!fs.existsSync(filepath)) {
      ...
      if (hasMagicFallback(filepath)) {
        ...
        const strExtensions = extensions.map((ext) => (ext.startsWith(".") ? ext : `.${ext}`)).join("|");
        pattern = `${filepath}+(${strExtensions})`;
        debugFallback("looking for files using glob pattern: %s", pattern);
      }
      let matched = [];
      try { matched = fs.globSync(pattern, { withFileTypes: false }); } catch (err) { debugFallback("fs.globSync failed for pattern %s: %O", pattern, err); matched = []; }
      matched = matched.filter((p) => { try { return !fs.statSync(p).isDirectory(); } catch { return true; } });
      matched.sort((a, b) => a.localeCompare(b, "en"));
      files.push(...matched);
      if (!files.length) throw createNoFilesMatchPatternError(`Cannot find any files matching pattern "${filepath}"`, filepath);
      return files;
    }
    try { stat = fs.statSync(filepath); if (stat.isFile() || stat.isFIFO()) return [filepath]; } catch { return; }
    fs.readdirSync(filepath).forEach((dirent) => {
      const pathname = pathFallback.join(filepath, dirent);
      let s;
      try { s = fs.statSync(pathname); if (s.isDirectory()) { if (recursive) files.push(...lookupFiles(pathname, extensions, recursive)); return; } } catch { return; }
      if (!extensions.length) throw createMissingArgumentError(`Argument '${extensions}' required when argument '${filepath}' is a directory`, "extensions", "array");
      if (!s.isFile() || !hasMatchingExtname(pathname, extensions) || isHiddenOnUnix(pathname)) return;
      files.push(pathname);
    });
    return files;
  };
