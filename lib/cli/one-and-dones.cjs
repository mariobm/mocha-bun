...
const showKeys = (obj) => {
  ...
  keys
    .filter((key) => /^[a-z]/.test(key) && !obj[key].browserOnly && !obj[key].abstract)
    .sort()
    .forEach((key) => {
...
const ONE_AND_DONES = {
  "list-interfaces": () => { showKeys(Mocha.interfaces); },
  "list-reporters": () => { showKeys(Mocha.reporters); },
};
const ONE_AND_DONE_ARGS = new Set(["help", "h", "version", "V"].concat(Object.keys(ONE_AND_DONES)));
