...
import { isNodeFlag } from "./node-flags.js";
 ...
 * Handles negative numbers and non-flag values (e.g. --timeout -1, --grep -1).
 */
const validateArgsBeforeParse = (allArgs) => {
  allArgs.forEach((arg, index) => {
    ...
    if (requiresValue(name)) {
      ...
      if (next.startsWith("-")) {
        // If next is a negative number, it's a valid value for any option that requires a value
        if (isNumeric(next)) {
          return;
        }
        // If next is not a known Mocha or Node flag, treat it as a value (e.g. --grep -1)
        if (!isMochaFlag(next) && !isNodeFlag(next, false)) {
          return;
        }
        throw new Error(`Not enough arguments following: ${name}`);
      }
...
export const parseMochaArgs = (
  ...
  const allArgs = Array.isArray(args) ? args : args ? splitShellArgs(args) : [];
