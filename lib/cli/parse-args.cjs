"use strict";
// Auto-generated CJS for Bun (original parse-args.js is ESM)
const { parseArgs } = require("node:util");
let errors;
try { errors = require("../errors.cjs"); } catch { errors = require("../errors.js"); }
const { createInvalidArgumentTypeError, createUnsupportedError } = errors;
let utilsCjs;
try { utilsCjs = require("../utils.cjs"); } catch { utilsCjs = require("../utils.cjs"); }
const { isNumeric } = utilsCjs;
let runHelpers;
try { runHelpers = require("./run-helpers.cjs"); } catch { runHelpers = require("./run-helpers.cjs"); }
const { list } = runHelpers;
let meta;
try { meta = require("./run-option-metadata.cjs"); } catch { meta = require("./run-option-metadata.cjs"); }
const { aliases, expectedTypeForFlag, isMochaFlag, stripLeadingDashes, types } = meta;
let nodeFlagsMod;
try { nodeFlagsMod = require("./node-flags.cjs"); } catch { nodeFlagsMod = require("./node-flags.cjs"); }
const { isNodeFlag } = nodeFlagsMod;
