...
let runHelpers;
try {
  runHelpers = require("./run-helpers.cjs");
} catch {
  runHelpers = require("./run-helpers.cjs");
}
...
let meta;
try {
  meta = require("./run-option-metadata.cjs");
} catch {
  meta = require("./run-option-metadata.cjs");
}
const { aliases, expectedTypeForFlag, isMochaFlag, stripLeadingDashes, types } =
  meta;
let nodeFlagsMod;
try {
  nodeFlagsMod = require("./node-flags.cjs");
} catch {
  nodeFlagsMod = require("./node-flags.cjs");
}
...
exports.parseMochaArgs = (args = [], defaultValues = {}, ...configObjects) => {
