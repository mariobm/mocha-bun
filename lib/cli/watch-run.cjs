...
/**
 * Detect if a path segment contains glob magic (replaces glob.hasMagic).
 * Covers *, ?, [, ], {, }, (, ), !, +, @
 * @param {string} str
 * @returns {boolean}
 */
function isGlobSegment(str) {
  return /[*?\[\]{}()!+@]/.test(str);
}
 ...
 * Glob pattern string (replaces glob.Glob pattern class after migrating to fs.glob).
...
function createPathFilter(globPaths, basePath) {
  ...
  // Expand braces using minimatch (replaces glob.Glob with magicalBraces)
  // Each globPath may expand to multiple patterns e.g. "a/{b,c}/*.js" -> ["a/b/*.js","a/c/*.js"]
  const patterns = globPaths.flatMap((globPath) => {
    try {
      return braceExpand(globPath);
    } catch {
      return [globPath];
    }
  });
  ...
  for (const patternStr of patterns) {
    debug("processing glob pattern: %s", patternStr);

    // Split pattern by forward slash (glob uses posix separators even on Windows)
    // Handle leading slash for absolute patterns
    const rawParts = patternStr.split("/");
    const segments = [];
    let isGlob = false;

    for (const part of rawParts) {
      if (part === "" ) { // leading slash or duplicate slashes; skip for segment collection but preserve for absPath
        if (segments.length === 0 && patternStr.startsWith("/")) {
          // absolute pattern: first empty part indicates root, keep segments empty for now
          continue;
        }
        if (part === "") continue;
      }
      debug(
        "found %s pattern: %s",
        isGlobSegment(part) ? "glob or regexp" : "string",
        part,
      );
      if (isGlobSegment(part)) {
        isGlob = true;
        break;
      }
      segments.push(part);
    ...
    if (!isGlob) {
      debug("all subpatterns of %j processed", patternStr);
    }
    ...
    const absPath = path.resolve(basePath, patternStr);
